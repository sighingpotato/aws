...
### **Q3. S3 Presigned URL 유효 기간(7일) 미적용 문제**
* **현상**: 코드상으로 URL 만료 시간을 7일(`Duration.ofDays(7)`)로 설정하고, 생성된 URL 내 `X-Amz-Expires=604800` 파라미터를 확인했음에도 불구하고 약 하루 뒤 "토큰 만료(ExpiredToken)" 에러와 함께 접근이 차단됨.
* **원인**: 
    - **IAM Role의 한계**: EC2 인스턴스에 부여된 IAM Role은 보안을 위해 **STS(Security Token Service)**를 통한 '임시 자격 증명'을 사용함.
    - **서명 유효성 제약**: Presigned URL은 해당 URL을 서명할 때 사용한 자격 증명(열쇠)의 수명을 초과할 수 없음. 즉, 코드에서 7일을 설정해도 서명에 사용된 임시 열쇠가 갱신되어 파기되면 URL도 함께 무효화됨.
* **해결**:
    - AWS 콘솔 내 **IAM Role 설정**에서 `최대 세션 지속 시간(Maximum session duration)`을 기본 1시간에서 **최대 12시간**으로 연장하여 URL의 실질적인 수명을 최대한 확보함.
    - **교훈**: Lv.4의 IAM Role 기반 아키텍처에서는 보안상의 이유로 물리적인 7일 유효 기간 유지가 어려움을 인지함. 영구적인 링크 유지가 필요한 경우 Lv.6에서 다룰 **CloudFront Signed URL** 도입이 근본적인 해결책임을 파악함.
---
